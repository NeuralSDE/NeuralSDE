...
class Evaluator:
    ...
    def evaluate(
        ...
        for i, batch in enumerate(eval_gen):
            ...
            # Compute the loss
            loss_output = self.calculate_loss(model_outputs, model.func, model.logit_range)
    ...
    def calculate_loss(
        ...
        if func == "f_nll" or func == "f_mse":
            flow_matching_loss = torch.log((results.target_vectors - results.reconstructed_vectors)**2 + 1.0e-3).mean() \
                -torch.log(torch.tensor(1.0e-3).to(results.target_vectors.device))
        elif func=="d":
            flow_matching_loss = nn.MSELoss()(results.reconstructed_vectors, results.target_vectors)
        elif func=="g":
            logit_min=logit_range[0]
            logit_max=logit_range[1]
            flow_matching_loss = nn.MSELoss()(
